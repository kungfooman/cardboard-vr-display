...
function createTypeFromIndexedAccess(expect, warn) {
  ...
  if (resolvedObject) {
    ...
    for (const indexKey of indexKeys) {
      ...
      if (!prop) {
        console.warn(`Missing prop for ${indexKey}`, {
          indexKey
        });
      }
...
function createTypeFromKeyof(expect, warn) {
  ...
  warn('createTypeFromKeyof: unhandled type.', {
    expect
  });
}
...
function validateIntersection(value, expect, loc, name, critical, warn, depth) {
  ...
  for (const member of members) {
    ...
    if (!good) {
      warn(`Doesn't match intersection member.`, {
        member
      });
      return false;
    }
...
function validateNumber(obj, prop) {
  ...
  if (val === null) {
    console.warn(`${type}#${prop} null`, {
      obj
    });
    return false;
  }
  if (val === undefined) {
    console.warn(`${type}#${prop} undefined`, {
      obj
    });
    return false;
  }
  if (isNaN(val)) {
    console.warn(`${type}#${prop} NaN`, {
      obj
    });
    return false;
  }
  if (!isFinite(val)) {
    console.warn(`${type}#${prop} +-Infinity`, {
      obj
    });
    return false;
  }
      ...
      Object.keys(value).forEach(key => {
        ...
        if (!properties[key]) {
          if (options.logSuperfluousProperty) {
            warn(`Superfluous property: ${name}.${key}`, {
              properties,
              value
            });
          }
...
function validateTuple(value, expect, loc, name, critical, warn, depth) {
  ...
  const ret = elements.every((element, i) => {
    return validateType(value[i], element, loc, `${name}[${i}]`, critical, warn, depth + 1);
  });
  if (!ret) {
...
function validateUnion(value, expect, loc, name, critical, warn, depth) {
  const ret = expect.members.some(member => validateType(value, member, loc, name + ` union type: ${(member == null ? void 0 : member.type) || member}`, false, () => undefined, depth + 1));
  if (!ret) {
...
let enabled = true;
function disableTypeChecking() {
  enabled = false;
}
function enableTypeChecking() {
  enabled = true;
}
/**
...
function validateType(value, expect, loc, name, critical = true, warn, depth) {
  ...
  if (type === 'class') {
    if (value && expect.elementType === 'ScriptType') {
      ...
      const proto = Object.getPrototypeOf(value);
      if ((proto == null ? void 0 : proto.name) === 'ScriptType') {
        return true;
      }
    }
  ...
  if (type === 'ResourceHandler') {
    ...
    return value == null || (_value$constructor = value.constructor) == null ? void 0 : _value$constructor.name.endsWith('Handler');
  }
  ...
  if (typeof window !== 'undefined') {
    ...
    if (type.startsWith('globalThis.')) {
      ...
      const _windowClass = window[innerType];
      if (_windowClass) {
        return value instanceof _windowClass;
      }
    }
  }
...
function partition(arr, fn) {
  ...
  for (const element of arr) {
    const _arr = fn(element) ? pass : fail;
    _arr.push(element);
  }
  return [pass, fail];
}
...
class Warning {
  constructor(msg, value, expect, loc, name) {
    ...
    this.td_dbg = document.createElement('td');
    this.td_hide = document.createElement('td');
    this.td_location = document.createElement('td');
    this.td_name = document.createElement('td');
    this.td_expect = document.createElement('td');
    this.td_value = document.createElement('td');
    this.td_count = document.createElement('td');
    this.td_desc = document.createElement('td');
    this.button_dbgInput = document.createElement('button');
    this.button_hideInput = document.createElement('button');
    this._msg = '';
    this._hits = 0;
    this._hidden = false;
    this._dbg = false;
...
class TypePanel {
  constructor() {
    ...
    this.spanErrors = document.createElement('span');
    this.span = document.createElement('span');
    this.select = document.createElement('select');
    this.option_spam = document.createElement('option');
    this.option_once = document.createElement('option');
    this.option_never = document.createElement('option');
    this.buttonHide = document.createElement('button');
    this.buttonLoadState = document.createElement('button');
    this.buttonSaveState = document.createElement('button');
    const {
      div,
      spanErrors,
      span,
  ...
  loadState() {
    ...
    for (const e of json) {
      ...
      if (!foundWarning) {
        ...
        warnedTable == null || warnedTable.append(foundWarning.tr);
...
function inspectType(value, expect, loc, name, critical = true) {
  ...
  if (!ret && critical) {
    ...
    const valueToString = value == null || value.toString == null ? void 0 : value.toString();
    ...
    if (!warnObj) {
      ...
      warnedTable == null || warnedTable.append(warnObj.tr);
...
function validateDivision(lhs, rhs) {
  ...
  if (!valid) {
    console.warn(`validateDivision> incompatible type pair`, {
      lhs,
      rhs,
      twoNumbers,
      twoBigInts
  ...
  if (isNaN(ret)) {
    console.warn(`validateDivision> NaN`, {
      lhs,
      rhs
    });
  }
  if (!isFinite(ret)) {
    console.warn(`validateDivision> +-Infinity`, {
      lhs,
      rhs
    });
  }
...
const isSafariWithoutDeviceMotion = (
  function () {
    let value = false;
...
const isChromeWithoutDeviceMotion = (
  function () {
    let value = false;
...
const orthoMatrix = function (out, left, right, bottom, top, near, far) {
  var lr = validateDivision(1, (left - right)), bt = validateDivision(1, (bottom - top)), nf = validateDivision(1, (near - far));
  out[0] = -2 * lr;
  out[1] = 0;
  out[2] = 0;
  out[3] = 0;
...
const frameDataFromPose = (
  function () {
    var piOver180 = validateDivision(Math.PI, 180.0);
    var rad45 = Math.PI * 0.25; // Borrowed from glMatrix.

    function mat4_perspectiveFromFieldOfView(out, fov, near, far) {
      var upTan = Math.tan(fov ? (fov.upDegrees * piOver180) : rad45), downTan = Math.tan(fov ? (fov.downDegrees * piOver180) : rad45), leftTan = Math.tan(fov ? (fov.leftDegrees * piOver180) : rad45), rightTan = Math.tan(fov ? (fov.rightDegrees * piOver180) : rad45), xScale = validateDivision(2.0, (leftTan + rightTan)), yScale = validateDivision(2.0, (upTan + downTan));
      out[0] = xScale;
      out[1] = 0.0;
      out[2] = 0.0;
      out[3] = 0.0;
      ...
      out[10] = validateDivision(far, (near - far));
      out[11] = -1.0;
      out[12] = 0.0;
      out[13] = 0.0;
      out[14] = validateDivision((far * near), (near - far));
      out[15] = 0.0;
      return out;
    }
...
const oldIOS = () => {
return typeof navigator !== "undefined" && parseFloat((
  "" + (
    /CPU.*OS ([0-9_]{3,4})[0-9_]{0,1}|(CPU like).*AppleWebKit.*Mobile/i.exec(navigator.userAgent) || [0, ""]
  )[1]
).replace("undefined", "3_2").replace("_", ".").replace("_", "")) < 10 && !window.MSStream;
};
...
registerClass(NoSleep);

registerTypedef('FieldOfView', {
  "type": "object",
  "properties": {
    "downDegrees": "number",
    "leftDegrees": "number",
...
function WGLUPreserveGLState(gl, bindings, callback) {
  if (!inspectType(gl, "WebGL2RenderingContext", 'WGLUPreserveGLState', 'gl')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(bindings, {
    "type": "array",
    "elementType": "any",
    "optional": false
  }, 'WGLUPreserveGLState', 'bindings')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(callback, "function", 'WGLUPreserveGLState', 'callback')) {
    youCanAddABreakpointHere();
  }
  if (!bindings) {
    callback(gl);
...
function CardboardDistorter(gl, cardboardUI, bufferScale, dirtySubmitFrameBindings) {
  if (!inspectType(gl, "WebGLRenderingContext", 'CardboardDistorter', 'gl')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(cardboardUI, {
    "type": "union",
    "members": [
      "CardboardUI",
      "null"
    ...
    "optional": false
  }, 'CardboardDistorter', 'cardboardUI')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(bufferScale, "number", 'CardboardDistorter', 'bufferScale')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(dirtySubmitFrameBindings, "boolean", 'CardboardDistorter', 'dirtySubmitFrameBindings')) {
    youCanAddABreakpointHere();
  }
  this.gl = gl;
  ...
  for (var e = 0; e < 2; e++) {
    for (var j = 0; j < height; j++) {
      for (var i = 0; i < width; i++, vidx++) {
        var u = validateDivision(i, (width - 1));
        var v = validateDivision(j, (height - 1));
        ...
        var p = validateDivision(x * r, d);
        var q = validateDivision(y * r, d);
        u = validateDivision((p - noLensFrustum[0]), (noLensFrustum[2] - noLensFrustum[0]));
        v = validateDivision((q - noLensFrustum[3]), (noLensFrustum[1] - noLensFrustum[3]));
        // Convert u,v to mesh screen coordinates.

        validateDivision(deviceInfo.device.widthMeters, deviceInfo.device.heightMeters);
...
CardboardDistorter.prototype.computeMeshIndices_ = function (width, height) {
  ...
  var halfwidth = validateDivision(width, 2);
  var halfheight = validateDivision(height, 2);
  var vidx = 0;
  var iidx = 0;
  for (var e = 0; e < 2; e++) {
    for (var j = 0; j < height; j++) {
...
var DEG2RAD = validateDivision(Math.PI, 180.0);
...
CardboardUI.prototype.listen = function (optionsCallback, backCallback) {
  ...
  this.listener = function (event) {
    var midline = validateDivision(canvas.clientWidth, 2);
    ...
    if (event.clientX < buttonSize && event.clientY < buttonSize) {
      backCallback(event);
    }
    }
  };
...
CardboardUI.prototype.onResize = function () {
  ...
  WGLUPreserveGLState(gl, glState, function (gl) {
    var vertices = [];
    var midline = validateDivision(gl.drawingBufferWidth, 2);
    ...
    var scalingRatio = validateDivision(gl.drawingBufferWidth, physicalPixels);
    var dps = scalingRatio * window.devicePixelRatio;
    var lineWidth = validateDivision(kCenterLineThicknessDp * dps, 2);
    var buttonSize = kButtonWidthDp * kTouchSlopFactor * dps;
    var buttonScale = validateDivision(kButtonWidthDp * dps, 2);
    ...
    self.gearOffset = (validateDivision(vertices.length, 2));
    ...
    self.gearVertexCount = (validateDivision(vertices.length, 2)) - self.gearOffset;
    // Build back arrow

    self.arrowOffset = (validateDivision(vertices.length, 2));
    function addArrowVertex(x, y) {
      vertices.push(buttonBorder + x, gl.drawingBufferHeight - buttonBorder - y);
    }
    var angledLineWidth = validateDivision(lineWidth, Math.sin(45 * DEG2RAD));
    ...
    addArrowVertex(angledLineWidth, buttonScale + lineWidth);
    addArrowVertex(kButtonWidthDp * dps, buttonScale + lineWidth);
    self.arrowVertexCount = (validateDivision(vertices.length, 2)) - self.arrowOffset;
    // Buffer data
...
Distortion.prototype.distortInverse = function (radius) {
  if (!inspectType(radius, "Number", 'Distortion.prototype.distortInverse', 'radius')) {
    youCanAddABreakpointHere();
  }
  ...
  while (Math.abs(r1 - r0) > 0.0001 /** 0.1mm */ ) {
    ...
    const r2 = r1 - dr1 * (validateDivision((r1 - r0), (dr1 - dr0)));
    r0 = r1;
    r1 = r2;
    dr0 = dr1;
  }
...
Distortion.prototype.distort = function (radius) {
  if (!inspectType(radius, "Number", 'Distortion.prototype.distort', 'radius')) {
    youCanAddABreakpointHere();
  }
 ...
 * limitations under the License.
 */
const degToRad = validateDivision(Math.PI, 180);
const radToDeg = validateDivision(180, Math.PI);
...
Vector3.prototype = {
  ...
  normalize: function () {
    ...
    if (scalar !== 0) {
      var invScalar = validateDivision(1, scalar);
...
Quaternion.prototype = {
  ...
  setFromEulerXYZ: function (x, y, z) {
    var c1 = Math.cos(validateDivision(x, 2));
    var c2 = Math.cos(validateDivision(y, 2));
    var c3 = Math.cos(validateDivision(z, 2));
    var s1 = Math.sin(validateDivision(x, 2));
    var s2 = Math.sin(validateDivision(y, 2));
    var s3 = Math.sin(validateDivision(z, 2));
  ...
  setFromAxisAngle: function (axis, angle) {
    ...
    var halfAngle = validateDivision(angle, 2), s = Math.sin(halfAngle);
    this.x = axis.x * s;
    this.y = axis.y * s;
    this.z = axis.z * s;
  ...
  slerp: function (qb, t) {
    ...
    var ratioA = validateDivision(Math.sin((1 - t) * halfTheta), sinHalfTheta), ratioB = validateDivision(Math.sin(t * halfTheta), sinHalfTheta);
...
registerTypedef('CardboardViewerParams', {
  "type": "object",
  "properties": {
    "id": "string",
    "label": "string",
...
function Device(params) {
  if (!inspectType(params, "DeviceParams", 'Device', 'params')) {
    youCanAddABreakpointHere();
  }
...
function DeviceInfo(deviceParams, additionalViewers) {
  if (!inspectType(deviceParams, "DeviceParams", 'DeviceInfo', 'deviceParams')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(additionalViewers, {
    "type": "array",
    "elementType": "CardboardViewerParams",
    "optional": false
  }, 'DeviceInfo', 'additionalViewers')) {
    youCanAddABreakpointHere();
  }
...
DeviceInfo.prototype.determineDevice_ = function (deviceParams) {
  ...
  var metersPerPixelX = validateDivision(METERS_PER_INCH, deviceParams.xdpi);
  var metersPerPixelY = validateDivision(METERS_PER_INCH, deviceParams.ydpi);
...
DeviceInfo.prototype.getDistortedFieldOfViewLeftEye = function () {
  ...
  var outerDist = validateDivision((device.widthMeters - viewer.interLensDistance), 2);
  var innerDist = validateDivision(viewer.interLensDistance, 2);
  var bottomDist = viewer.baselineLensDistance - device.bevelMeters;
  var topDist = device.heightMeters - bottomDist;
  var outerAngle = radToDeg * Math.atan(distortion.distort(validateDivision(outerDist, eyeToScreenDistance)));
  var innerAngle = radToDeg * Math.atan(distortion.distort(validateDivision(innerDist, eyeToScreenDistance)));
  var bottomAngle = radToDeg * Math.atan(distortion.distort(validateDivision(bottomDist, eyeToScreenDistance)));
  var topAngle = radToDeg * Math.atan(distortion.distort(validateDivision(topDist, eyeToScreenDistance)));
  return {
...
DeviceInfo.prototype.getLeftEyeVisibleTanAngles = function () {
  ...
  var halfWidth = validateDivision(device.widthMeters, 4);
  var halfHeight = validateDivision(device.heightMeters, 2);
  // Viewport center, measured from left lens position.

  var verticalLensOffset = (viewer.baselineLensDistance - device.bevelMeters - halfHeight);
  var centerX = validateDivision(viewer.interLensDistance, 2) - halfWidth;
  var centerY = -verticalLensOffset;
  var centerZ = viewer.screenLensDistance;
  // Tan-angles of the viewport edges, as seen through the lens.

  var screenLeft = distortion.distort(validateDivision((centerX - halfWidth), centerZ));
  var screenTop = distortion.distort(validateDivision((centerY + halfHeight), centerZ));
  var screenRight = distortion.distort(validateDivision((centerX + halfWidth), centerZ));
  var screenBottom = distortion.distort(validateDivision((centerY - halfHeight), centerZ));
...
DeviceInfo.prototype.getLeftEyeNoLensTanAngles = function () {
  ...
  var screenLeft = validateDivision((centerX - halfWidth), centerZ);
  var screenTop = validateDivision((centerY + halfHeight), centerZ);
  var screenRight = validateDivision((centerX + halfWidth), centerZ);
  var screenBottom = validateDivision((centerY - halfHeight), centerZ);
  // Compare the two sets of tan-angles and take the value closer to zero on each side.
...
DeviceInfo.prototype.getLeftEyeVisibleScreenRect = function (undistortedFrustum) {
  ...
  var eyeX = validateDivision((device.widthMeters - viewer.interLensDistance), 2);
  var eyeY = viewer.baselineLensDistance - device.bevelMeters;
  var left = validateDivision((undistortedFrustum[0] * dist + eyeX), device.widthMeters);
  var top = validateDivision((undistortedFrustum[1] * dist + eyeY), device.heightMeters);
  var right = validateDivision((undistortedFrustum[2] * dist + eyeX), device.widthMeters);
  var bottom = validateDivision((undistortedFrustum[3] * dist + eyeY), device.heightMeters);
  return {
    x: left,
    y: bottom,
    width: right - left,
...
DeviceInfo.prototype.getUndistortedViewportLeftEye = function () {
  ...
  // noted.
  var eyeToScreenDistance = viewer.screenLensDistance;
  var screenWidth = validateDivision(device.widthMeters, eyeToScreenDistance);
  var screenHeight = validateDivision(device.heightMeters, eyeToScreenDistance);
  var xPxPerTanAngle = validateDivision(device.width, screenWidth);
  var yPxPerTanAngle = validateDivision(device.height, screenHeight);
...
DeviceInfo.prototype.getUndistortedParams_ = function () {
  ...
  var eyeToScreenDistance = viewer.screenLensDistance;
  var halfLensDistance = validateDivision(validateDivision(viewer.interLensDistance, 2), eyeToScreenDistance);
  var screenWidth = validateDivision(device.widthMeters, eyeToScreenDistance);
  var screenHeight = validateDivision(device.heightMeters, eyeToScreenDistance);
  var eyePosX = validateDivision(screenWidth, 2) - halfLensDistance;
  var eyePosY = validateDivision((viewer.baselineLensDistance - device.bevelMeters), eyeToScreenDistance);
...
function CardboardViewer(params) {
  if (!inspectType(params, "CardboardViewerParams", 'CardboardViewer', 'params')) {
    youCanAddABreakpointHere();
  }
...
registerTypedef('Device', {
  "type": "object",
  "properties": {
    "type": "string",
    "rules": {
      "type": "array",
      "elementType": "DpdbRule",
      "optional": false
    },
    "dpi": {
      "type": "union",
...
function Dpdb(url, onDeviceParamsUpdated) {
  if (!inspectType(url, "string", 'Dpdb', 'url')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(onDeviceParamsUpdated, "function", 'Dpdb', 'onDeviceParamsUpdated')) {
    youCanAddABreakpointHere();
  }
...
Dpdb.prototype.ruleMatches_ = function (rule, ua, screenWidth, screenHeight) {
  if (!inspectType(rule, "DpdbRule", 'Dpdb.prototype.ruleMatches_', 'rule')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(ua, "string", 'Dpdb.prototype.ruleMatches_', 'ua')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(screenWidth, "number", 'Dpdb.prototype.ruleMatches_', 'screenWidth')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(screenHeight, "number", 'Dpdb.prototype.ruleMatches_', 'screenHeight')) {
    youCanAddABreakpointHere();
  }
...
function SensorSample(sample, timestampS) {
  if (!inspectType(sample, "MathUtil.Vector3", 'SensorSample', 'sample')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(timestampS, "number", 'SensorSample', 'timestampS')) {
    youCanAddABreakpointHere();
  }
...
ComplementaryFilter.prototype.addGyroMeasurement = function (vector, timestampS) {
  if (!inspectType(vector, "MathUtil.Vector3", 'ComplementaryFilter.prototype.addGyroMeasurement', 'vector')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(timestampS, "number", 'ComplementaryFilter.prototype.addGyroMeasurement', 'timestampS')) {
    youCanAddABreakpointHere();
  }
...
ComplementaryFilter.prototype.accelToQuaternion_ = function (accel) {
  if (!inspectType(accel, "MathUtil.Vector3", 'ComplementaryFilter.prototype.accelToQuaternion_', 'accel')) {
    youCanAddABreakpointHere();
  }
...
ComplementaryFilter.prototype.gyroToQuaternionDelta_ = function (gyro, dt) {
  if (!inspectType(gyro, "MathUtil.Vector3", 'ComplementaryFilter.prototype.gyroToQuaternionDelta_', 'gyro')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(dt, "number", 'ComplementaryFilter.prototype.gyroToQuaternionDelta_', 'dt')) {
    youCanAddABreakpointHere();
  }
...
function PosePredictor(predictionTimeS, isDebug) {
  if (!inspectType(predictionTimeS, "Number", 'PosePredictor', 'predictionTimeS')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(isDebug, "boolean", 'PosePredictor', 'isDebug')) {
    youCanAddABreakpointHere();
  }
...
PosePredictor.prototype.getPrediction = function (currentQ, gyro, timestampS) {
  if (!inspectType(currentQ, "MathUtil.Quaternion", 'PosePredictor.prototype.getPrediction', 'currentQ')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(gyro, "MathUtil.Vector3", 'PosePredictor.prototype.getPrediction', 'gyro')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(timestampS, "number", 'PosePredictor.prototype.getPrediction', 'timestampS')) {
    youCanAddABreakpointHere();
  }
...
function FusionPoseSensor(kFilter, predictionTime, yawOnly, isDebug) {
  if (!inspectType(kFilter, "number", 'FusionPoseSensor', 'kFilter')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(predictionTime, "number", 'FusionPoseSensor', 'predictionTime')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(yawOnly, "boolean", 'FusionPoseSensor', 'yawOnly')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(isDebug, "boolean", 'FusionPoseSensor', 'isDebug')) {
    youCanAddABreakpointHere();
  }
  ...
  if (isIOS()) {
    this.filterToWorldQ.setFromAxisAngle(new Vector3(1, 0, 0), validateDivision(Math.PI, 2));
  } else {
    this.filterToWorldQ.setFromAxisAngle(new Vector3(1, 0, 0), validateDivision(-Math.PI, 2));
  }
  ...
  this.originalPoseAdjustQ.setFromAxisAngle(new Vector3(0, 0, 1), validateDivision(-window.orientation * Math.PI, 180));
...
FusionPoseSensor.prototype.getOrientation = function () {
  ...
  if (this.isWithoutDeviceMotion && this._deviceOrientationQ) {
    ...
    this.deviceOrientationFixQ = this.deviceOrientationFixQ || (
      function () {
        ...
        if (window.orientation === -90) {
          y.setFromAxisAngle(new Vector3(0, 1, 0), validateDivision(Math.PI, -2));
        } else {
          y.setFromAxisAngle(new Vector3(0, 1, 0), validateDivision(Math.PI, 2));
        }
        return z.multiply(y);
    ...
    this.deviceOrientationFilterToWorldQ = this.deviceOrientationFilterToWorldQ || (
      function () {
        const q = new Quaternion();
        q.setFromAxisAngle(new Vector3(1, 0, 0), validateDivision(-Math.PI, 2));
        return q;
      }
    )();
...
FusionPoseSensor.prototype.onDeviceOrientation_ = function (e) {
  ...
  alpha = validateDivision((alpha || 0) * Math.PI, 180);
  beta = validateDivision((beta || 0) * Math.PI, 180);
  gamma = validateDivision((gamma || 0) * Math.PI, 180);
...
FusionPoseSensor.prototype.updateDeviceMotion_ = function (deviceMotion) {
  ...
  var timestampS = validateDivision(deviceMotion.timeStamp, 1000);
  ...
  if (rotRate) {
    ...
    if (!this.isDeviceMotionInRadians) {
      this.gyroscope.multiplyScalar(validateDivision(Math.PI, 180));
    }
...
FusionPoseSensor.prototype.setScreenTransform_ = function () {
  ...
  break;
  case 90:
  this.worldToScreenQ.setFromAxisAngle(new Vector3(0, 0, 1), validateDivision(-Math.PI, 2));
  break;
  case -90:
  this.worldToScreenQ.setFromAxisAngle(new Vector3(0, 0, 1), validateDivision(Math.PI, 2));
  break;
...
SENSOR_TO_VR.setFromAxisAngle(X_AXIS, validateDivision(-Math.PI, 2));
SENSOR_TO_VR.multiply(new Quaternion().setFromAxisAngle(Z_AXIS, validateDivision(Math.PI, 2)));
...
ViewerSelector.prototype.createDialog_ = function (options) {
  ...
  s.top = '24px';
  s.left = '50%';
  s.marginLeft = (validateDivision(-width, 2)) + 'px';
  s.width = width + 'px';
  s.padding = '24px';
  s.overflow = 'hidden';
  s.background = '#fafafa';
...
ViewerSelector.prototype.createH1_ = function (name) {
  if (!inspectType(name, "string", 'ViewerSelector.prototype.createH1_', 'name')) {
    youCanAddABreakpointHere();
  }
...
ViewerSelector.prototype.createButton_ = function (label, onclick) {
  if (!inspectType(label, "string", 'ViewerSelector.prototype.createButton_', 'label')) {
    youCanAddABreakpointHere();
  }
  if (!inspectType(onclick, "function", 'ViewerSelector.prototype.createButton_', 'onclick')) {
    youCanAddABreakpointHere();
  }
...
CardboardVRDisplay.prototype.beginPresent_ = function () {
  ...
  if (!gl) {
  return;
  } // Can't do distortion without a WebGL context.
...
CardboardVRDisplay.prototype.submitFrame = function (pose) {
  ...
  if (this.cardboardUI_ && this.layer_) {
...
export { CardboardDistorter, CardboardUI, CardboardVRDisplay, DeviceInfo, Distortion, Dpdb, MAX_TIMESTEP, MIN_TIMESTEP, PoseSensor, Quaternion, RotateInstructions, TypePanel, VRDisplay, VRDisplayCapabilities, VRFrameData, Vector2, Vector3, ViewerSelector, Warning, assertMode, breakpointID, clamp, classes, clearObject, config, copyArray, createTable, createType, createTypeFromIndexedAccess, createTypeFromKeyof, createTypeFromMapping, customTypes, customValidations, dataUri, degToRad, deprecateWarning, disableTypeChecking, enableTypeChecking, enabled, exitFullscreen, extend, frameDataFromPose, getChromeVersion, getFullscreenElement, getOriginFromUrl, getProgramUniforms, getQuaternionAngle, getScreenHeight, getScreenWidth, getTypeKeys, inspectType, isChromeWithoutDeviceMotion, isFirefoxAndroid, isIOS, isInsideCrossOriginIFrame, isLandscapeMode, isMobile, isObject, isR7, isSafari, isSafariWithoutDeviceMotion, isTimestampDeltaValid, isWebViewAndroid, jsdocCache, lerp, linkProgram, makeJSDoc, options, orthoMatrix, partition, radToDeg, registerClass, registerTypedef, registerVariable, replaceType, report, requestFullscreen, reset, resolveType, safariCssSizeWorkaround, typePanel, typedefs, validateArray, validateDivision, validateIntersection, validateKeyof, validateMap, validateMapping, validateNumber, validateObject, validateRecord, validateTuple, validateType, validateTypedef, validateUnion, variables, warnOnce, warnedTable, youCanAddABreakpointHere };
